...
Schema contract in [SCHEMA.md](./SCHEMA.md). Codegen reference in [TRANSLATIONS.md](./TRANSLATIONS.md). Overall strategy in [STRATEGY.md](./STRATEGY.md)
...
Choosing option 3 with **runner-first as canonical**. Codegen deferred until paid for. Rationale lives in [STRATEGY.md](./STRATEGY.md) (decomposition as substrate, spec-as-contract, leverage-maximizing split).
...
- A live variables panel in the inspector shows captures as they fire; a transcript panel shows agent/user turns.
- Stopping the run cleanly disconnects audio, stops the runner, and clears the runtime overlay.
- The full loop runs against the existing `public/valentina.json` spec without code changes.
...
```
   ...
   |  ↑ variables panel      |  ↑ variables     |   |  - WS   /audio          |
   |  ↑ transcript panel     |                  |   |                         |
   +-------------------------+------------------+   |  Pipecat pipeline       |
                                                    |  ┌───────────────────┐  |
   Browser audio glue (~50 LOC):                    |  │ ws_in → vad → stt │  |
   - getUserMedia mic capture                       |  │ → context_aggreg  │  |
   - WebSocket frames to /audio                     |  │ → DISPATCHER      │  |
   - audio element for TTS playback                 |  │ → llm → tts → ws  │  |
                                                    |  └───────────────────┘  |
                                                    |                         |
                                                    |  Spec interpreter:      |
                                                    |  - flow_state           |
...
uxflows-runner/
  ...
  uxflows_runner/
    __init__.py
    cli.py                       # `uxflows-runner serve --spec path.json`
    spec/
    ...
    events/
      schema.py                  # pydantic event types — the editor contract
      emitter.py                 # SSE broker, fan-out to subscribers
    server/
      app.py                     # FastAPI: /run, /stop, /events, /audio, /health
      pipeline.py                # Pipecat pipeline construction
    config.py                    # env vars, provider stack selection
  examples/
    valentina/
      execution.json             # provider keys + voice IDs (gitignored)
  tests/
    test_methods.py
    test_routing.py
    test_captures.py
...
- `lib/store/runtime.ts` — zustand slice: `{connected, currentFlowId, recentFlowIds, variables, transcript, error}`
- `components/toolbar/RunControl.tsx` — Run/Stop button + status indicator
- `components/runtime/VariablesPanel.tsx` — live captures pane
- `components/runtime/TranscriptPanel.tsx` — agent/user turns pane
- Updates to [components/canvas/FlowNode.tsx](./components/canvas/FlowNode.tsx) — render `data.runtimeState: "active" | "recent" | null`
- Updates to [components/canvas/Canvas.tsx](./components/canvas/Canvas.tsx) — edge pulse on `exit_path_taken`

## Design decisions

### The dispatcher is canonical, audio infra is rented

The dispatcher implements the three-method substrate described in [AGENTS.md](./AGENTS.md). Each routing decision, capture, and condition is dispatched through one of:
...
A small TS module in the editor establishes a WebSocket to `/audio` on Run, forwards mic frames, plays back TTS frames. Pipecat ships a reference browser client; we adapt it minimally. No frontend audio processing of our own — Pipecat's pipeline handles VAD, format conversion, etc.

### Provider stack default

For v0 development:

- **STT**: Deepgram Nova-3 (proven streaming, low latency, generous free tier).
- **LLM**: OpenAI gpt-4o (function-tool schema is best-documented; switch to Anthropic later).
- **TTS**: Cartesia Sonic (low first-byte latency, streaming).

All keys held in `examples/valentina/execution.json`, gitignored. Provider abstraction lives in `config.py`; swap is a one-line change.
...
### Phase 0 — hello-world bot in browser (1–2 days)

Stand up the audio loop end-to-end with a hardcoded prompt. No spec involvement yet.

- Create `uxflows-runner` repo skeleton, `pyproject.toml`, basic FastAPI app.
- Pipecat pipeline with `WebsocketServerTransport`, SileroVAD, Deepgram STT, OpenAI LLM (single hardcoded system prompt), Cartesia TTS.
- Adapt Pipecat's reference browser client; embed in the editor at `components/runtime/AudioBridge.tsx`. Browser opens WS to `ws://localhost:8000/audio`; mic in, TTS out.
- `.env.example` with the three keys; README with quickstart.
- **Acceptance:** click Run, grant mic, hear an agent greeting, talk to it, get a coherent reply. No spec, no canvas highlight.

### Phase 1 — v0 dispatcher (3–5 days)

Replace the hardcoded prompt with a spec-driven flow interpreter.

- `spec/types.py` — pydantic models mirroring SCHEMA.md v0. Validate against `public/valentina.json` as the test fixture.
...
- **Acceptance:** Run against `valentina.json`. The agent's first turn matches the entry flow's `instructions`. Captures fire. Routing transitions to the right next flow. CLI logs show events. Canvas not yet wired.

### Phase 2 — editor wiring + canvas highlight (2–3 days)

The visual payoff phase. The infrastructure already exists; this phase connects it.
...
- `events/emitter.py` — SSE broker class. `subscribe(session_id) -> AsyncIterator[Event]`. Fan-out to multiple subscribers (the editor, eventually replay tools).
- `server/app.py` — `GET /events?session_id=...` SSE endpoint; `POST /run` returns `{session_id}` and starts the pipeline; `POST /stop` ends the session.
...
- `components/toolbar/RunControl.tsx` — button reading from `runtime.connected`; click → fetch `/run`, then open SSE; second click → fetch `/stop`.
- [components/canvas/FlowNode.tsx](./components/canvas/FlowNode.tsx) — read `data.runtimeState`, render border ring (active = pulsing emerald, recent = fading emerald, none = current behavior). React Flow re-renders nodes on `data` change.
- [components/canvas/Canvas.tsx](./components/canvas/Canvas.tsx) — on `exit_path_taken` event, set the matching edge's `data.pulse = true` for ~600ms then unset.
- Inspector tabs gain a "Live" tab (visible only when `runtime.connected`) showing variables and transcript.
- **Acceptance:** click Run → canvas lights up the entry flow → talk to the agent → watch nodes change as flows transition. Edges pulse on routing decisions. Variables stream in.
...
- Run a real spec end-to-end (the Valentina spec or a scratch one); document anything that surprised us in this file.
