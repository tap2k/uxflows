...
**Variables** — spontaneously generated by use; no declaration required. A variable exists because something references it. Optional `variables` dictionary at agent and flow level enriches existing variables with `type`, `scope`, `description`, and `example`.
...
- **`variables`** — optional dictionary of variable declarations that enrich already-existing variables with `type`, `scope`, `description`, and `example`. Declaration does not create a variable; only reference does. See [Variables](#variables) for the scope semantics and why declarations are optional.
